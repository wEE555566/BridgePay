...
export async function POST(req, { params }) {
  try {
    ...
    if (room.sellerId?.toString() !== session.user.id) {
       return NextResponse.json({ message: "เฉพาะผู้ขายเท่านั้นที่สามารถส่งสินค้าได้" }, { status: 403 });
    }

    if (room.status === 'PAID') {
      room.status = 'SHIPPED';
      room.trackingNumber = trackingNumber;
      await room.save();
      return NextResponse.json({ message: "อัปเดตสถานะจัดส่งแล้ว", room }, { status: 200 });
    }
